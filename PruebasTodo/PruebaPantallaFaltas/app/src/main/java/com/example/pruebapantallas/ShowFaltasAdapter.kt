...
class ShowFaltasAdapter(private val faltas:LinkedHashSet<Falta>): RecyclerView.Adapter<ShowFaltasAdapter.ItemViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ShowFaltasAdapter.ItemViewHolder, position: Int) {
        ...
        holder.itemView.findViewById<ImageButton>(R.id.ivEliminar).setOnClickListener {
            ...
            if (absolutePosition != RecyclerView.NO_POSITION) { // Verificar si la posición es válida
                confirmAndRemoveFalta(absolutePosition, holder.itemView.context)
            }
        }
    }
    ...
    fun confirmAndRemoveFalta(position: Int, context: Context) {
    ...
    inner class ItemViewHolder(private val binding: ShowfaltaItemBinding): RecyclerView.ViewHolder(binding.root) {
